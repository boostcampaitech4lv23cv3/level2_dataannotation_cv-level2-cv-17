...
def cal_distance(x1, y1, x2, y2):
    logger.info(f"cal_distance({type(x1)}, {type(y1)}, {type(x2)}, {type(y2)})")
    """calculate the Euclidean distance"""
    return math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)


def move_points(vertices: np.ndarray, index1, index2, r, coef):
    ...
    """
    logger.info(
        f"move_points({type(vertices)}, {type(index1)}, {type(index2)}, {type(r)}, {type(coef)})"
    )
    index1 = index1 % 4
    index2 = index2 % 4
    x1_index = index1 * 2 + 0
...
def shrink_poly(vertices: np.ndarray, coef: float = 0.3):
...
def rotate_vertices(
    vertices: np.ndarray, theta: float, anchor: Union[np.ndarray, None] = None
):
...
def get_boundary(vertices: np.ndarray):
...
def cal_error(vertices: np.ndarray):
...
def is_cross_text(start_loc: list, length: int, vertices: np.ndarray):
...
def crop_img(img: Image.Image, vertices: np.ndarray, labels: np.ndarray, length: int):
...
def resize_img(img: JpegImagePlugin.JpegImageFile, vertices: np.ndarray, size: int):
    h, w = img.height, img.width
    ratio = size / max(h, w)
    if w > h:
...
def adjust_height(img: Image.Image, vertices: np.ndarray, ratio: float = 0.2):
...
def rotate_img(img: Image.Image, vertices: np.ndarray, angle_range=10):
...
def filter_vertices(
    vertices: np.ndarray, labels: np.ndarray, ignore_under: int = 0, drop_under: int = 0
):
...
class SceneTextDataset(Dataset):
    def __init__(
        ...
        with open(osp.join(root_dir, f"ufo/{split}.json")) as f:
            anno = json.load(f)
