...
class EvalHyperParams:
    AREA_RECALL_CONSTRAINT: float = 0.8
    AREA_PRECISION_CONSTRAINT: float = 0.4
    EV_PARAM_IND_CENTER_DIFF_THR: int = 1
    MTYPE_OO_O: float = 1.0
    MTYPE_OM_O: float = 0.8
    MTYPE_OM_M: float = 1.0
    GT_SAMPLE_NAME_2_ID: str = "gt_img_([0-9]+).txt"
    DET_SAMPLE_NAME_2_ID: str = "res_img_([0-9]+).txt"
    CRLF: bool = platform.system() == "Windows"
...
def calc_deteval_metrics(
    ...
    eval_hparams: EvalHyperParams = None,
    bbox_format="rect",
    verbose=False,
):
    """
    ...
    """
    logger.info(
        f"calc_deteval_metrics({type(pred_bboxes_dict)}, {type(gt_bboxes_dict)},",
        f"{type(transcriptions_dict)}, {type(eval_hparams)}, {type(bbox_format)},",
        f"{type(verbose)})",
    )
