# Examples:
#   $ python ufo_visualizer.py
#   $ python ufo_visualizer.py --root_dir /opt/ml/data/campers \
#         --annotation_filename annotation --port 30002
#
# Reference:
#   - 3기 CV-19조 GitHub의 visualization.py
#     github.com/boostcampaitech3/level2-data-annotation_cv-level2-cv-19
...
def draw_polygon(img: Image, pts: Sequence[point], illegibility: bool):
    ...
    img_draw.line(
        pts, width=3, fill=(0, 255, 255) if not illegibility else (255, 0, 255)
    )


# -- load annotations
filename = args.annotation_filename + ".json"
filepath = os.path.join(args.root_dir, "ufo", filename)
...
def update_img(btn_n, btn_p, btn_g, go_idx: int):
    ...
    title = f"[{img_idx+1}/{num_files}] {os.path.basename(img_path)}"
    return fig, title, img_idx
