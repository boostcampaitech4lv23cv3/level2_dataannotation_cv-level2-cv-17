...
def get_language_token(x):
    logger.info(f"get_language_token({type(x)})")
    return LANGUAGE_MAP.get(x, "others")


# @log(my_logger=logger)
def maybe_mkdir(x):
    logger.info(f"maybe_mkdir({type(x)})")
    if not osp.exists(x):
        os.makedirs(x)
...
class MLT17Dataset(Dataset):
    def __init__(self, image_dir, label_dir, copy_images_to=None):
        logger.info(
            f"MLT17Dataset({type(image_dir)}, {type(label_dir)}, {type(copy_images_to)})"
        )
        image_paths = {
            x
    ...
    def parse_label_file(self, label_path):
        logger.info(f"parse_label_file({type(label_path)})")

        def rearrange_points(points):
            logger.info(f"rearrange_points({type(points)})")
