...
class GeoTransformation:
    ...
    def __init__(
        ...
        if type(crop_size) in [int, float]:
            ...
            assert isinstance(crop_size[0], crop_size[1])
        ...
        if resize_to is None:
            ...
            assert isinstance(resize_to[0], resize_to[1])
    ...
    def crop_rotate_resize(
        ...
        Args:
            image (ndarray): (H, W, C) shaped ndarray.
            masks (List[ndarray]):
                List of (H, W) shaped ndarray the same size as the image.
        """
