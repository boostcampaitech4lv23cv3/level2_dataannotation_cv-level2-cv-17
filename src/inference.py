...
def do_inference(model, ckpt_fpath, data_dir, input_size, batch_size, split="public"):
    logger.info(
        f"do_inference({type(model)}, {type(ckpt_fpath)}, {type(data_dir)},{type(input_size)}, {type(batch_size)}, {type(split)})"
    )
    ...
    images = []
    for image_fpath in tqdm(glob(osp.join(data_dir, f"{split}/*"))):
